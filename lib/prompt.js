...
// U6: curated starter pack — pre-selected to reduce noise for first-time users
const STARTER_PACK = new Set(["review", "scan-secrets", "audit-deps"]);

async function askSkills(rl, skillSet) {
  const selected = skillSet.map((s) => STARTER_PACK.has(s.slug));
