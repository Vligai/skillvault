"use strict";

const crypto = require("crypto");
const fs = require("fs");
const path = require("path");

const PKG_ROOT = path.join(__dirname, "..");

const SKILLS = [
  { name: "Code Security Reviewer", slug: "review", category: "developer", file: "code-security-reviewer.md", command: "review.md" },
  { name: "Secret Scanner", slug: "scan-secrets", category: "developer", file: "secret-scanner.md", command: "scan-secrets.md" },
  { name: "Dependency Auditor", slug: "audit-deps", category: "developer", file: "dependency-auditor.md", command: "audit-deps.md" },
  { name: "Input Validation Generator", slug: "gen-validation", category: "developer", file: "input-validation-generator.md", command: "gen-validation.md" },
  { name: "CI/CD Security Hardener", slug: "harden-cicd", category: "developer", file: "cicd-security-hardener.md", command: "harden-cicd.md" },
  { name: "Auth Flow Scaffolder", slug: "scaffold-auth", category: "developer", file: "auth-flow-scaffolder.md", command: "scaffold-auth.md" },
  { name: "Dockerfile Security Linter", slug: "lint-dockerfile", category: "developer", file: "dockerfile-security-linter.md", command: "lint-dockerfile.md" },
  { name: "SBOM Generator", slug: "gen-sbom", category: "developer", file: "sbom-generator.md", command: "gen-sbom.md" },
  { name: "Secure API Design Reviewer", slug: "review-api", category: "developer", file: "secure-api-design-reviewer.md", command: "review-api.md" },
  { name: "Threat Model Generator", slug: "threat-model", category: "security", file: "threat-model-generator.md", command: "threat-model.md" },
  { name: "Incident Response Playbook Builder", slug: "ir-playbook", category: "security", file: "incident-response-playbook-builder.md", command: "ir-playbook.md" },
  { name: "Hardening Checklist Generator", slug: "harden-checklist", category: "security", file: "hardening-checklist-generator.md", command: "harden-checklist.md" },
  { name: "Detection Rule Generator", slug: "gen-detection-rule", category: "security", file: "detection-rule-generator.md", command: "gen-detection-rule.md" },
  { name: "CVE Analyzer", slug: "analyze-cve", category: "security", file: "cve-analyzer.md", command: "analyze-cve.md" },
  { name: "IAM Policy Analyzer", slug: "analyze-iam", category: "cloud", file: "iam-policy-analyzer.md", command: "analyze-iam.md" },
  { name: "IaC Scanner", slug: "scan-iac", category: "cloud", file: "iac-scanner.md", command: "scan-iac.md" },
  { name: "Phishing Email Analyzer", slug: "check-phishing", category: "user", file: "phishing-email-analyzer.md", command: "check-phishing.md" },
  { name: "PR Security Review Checklist", slug: "pr-checklist", category: "team", file: "pr-security-checklist.md", command: "pr-checklist.md" },
];

// ── Platform Registry ────────────────────────────────────────────────────────

const PLATFORMS = [
  {
    key: "claude", name: "Claude Code", flag: "claude",
    detect: [{ type: "dir", path: ".claude" }],
    type: "claude-commands",
  },
  {
    key: "cursor", name: "Cursor", flag: "cursor",
    detect: [{ type: "dir", path: ".cursor" }],
    type: "rules-dir", skillsDir: ".cursor/rules",
    guardrailsTarget: ".cursor/rules/security-guardrails.md",
  },
  {
    key: "windsurf", name: "Windsurf", flag: "windsurf",
    detect: [{ type: "dir", path: ".windsurf" }],
    type: "rules-dir", skillsDir: ".windsurf/rules",
    guardrailsTarget: ".windsurf/rules/security-guardrails.md",
  },
  {
    key: "copilot", name: "GitHub Copilot", flag: "copilot",
    detect: [{ type: "file", path: ".github/copilot-instructions.md" }],
    type: "rules-dir", skillsDir: ".github/copilot/skills",
    guardrailsAppendTarget: ".github/copilot-instructions.md",
  },
  {
    key: "cline", name: "Cline", flag: "cline",
    detect: [{ type: "dir", path: ".cline" }],
    type: "rules-dir", skillsDir: ".cline/rules",
    guardrailsTarget: ".cline/rules/security-guardrails.md",
  },
  {
    key: "roo", name: "Roo Code", flag: "roo",
    detect: [{ type: "dir", path: ".roo" }],
    type: "rules-dir", skillsDir: ".roo/rules",
    guardrailsTarget: ".roo/rules/security-guardrails.md",
  },
  {
    key: "continue", name: "Continue", flag: "continue",
    detect: [{ type: "dir", path: ".continue" }],
    type: "rules-dir", skillsDir: ".continue/rules",
    guardrailsTarget: ".continue/rules/security-guardrails.md",
  },
  {
    key: "codex", name: "Codex CLI", flag: "codex",
    detect: [{ type: "file", path: "AGENTS.md" }],
    type: "rules-dir", skillsDir: "codex-skills",
    guardrailsAppendTarget: "AGENTS.md",
  },
  {
    key: "amazonq", name: "Amazon Q Developer", flag: "amazonq",
    detect: [{ type: "dir", path: ".q" }],
    type: "rules-dir", skillsDir: ".q/rules",
    guardrailsTarget: ".q/rules/security-guardrails.md",
  },
  {
    key: "cody", name: "Sourcegraph Cody", flag: "cody",
    detect: [{ type: "dir", path: ".cody" }],
    type: "rules-dir", skillsDir: ".cody/rules",
    guardrailsTarget: ".cody/rules/security-guardrails.md",
  },
  {
    key: "jetbrains", name: "JetBrains AI", flag: "jetbrains",
    detect: [{ type: "dir", path: ".junie" }],
    type: "rules-dir", skillsDir: ".junie/guidelines",
    guardrailsTarget: ".junie/guidelines/security-guardrails.md",
  },
  {
    key: "tabnine", name: "Tabnine", flag: "tabnine",
    detect: [{ type: "dir", path: ".tabnine" }],
    type: "rules-dir", skillsDir: ".tabnine/rules",
    guardrailsTarget: ".tabnine/rules/security-guardrails.md",
  },
  {
    key: "aider", name: "Aider", flag: "aider",
    detect: [{ type: "file", path: ".aider.conf.yml" }],
    type: "rules-dir", skillsDir: ".aider/rules",
    guardrailsTarget: ".aider/rules/security-guardrails.md",
  },
  {
    key: "augment", name: "Augment Code", flag: "augment",
    detect: [{ type: "file", path: "augment-guidelines.md" }],
    type: "rules-dir", skillsDir: "augment-skills",
    guardrailsAppendTarget: "augment-guidelines.md",
  },
];

function getVersion() {
  const pkg = JSON.parse(fs.readFileSync(path.join(PKG_ROOT, "package.json"), "utf8"));
  return pkg.version;
}

function doctor(targetDir) {
  const version = getVersion();
  const detected = detectPlatforms(targetDir);
  const skills = listSkills(targetDir);

  const platforms = {};
  for (const platform of PLATFORMS) {
    const isDetected = detected[platform.key];
    let skillCount = 0;
    let guardrails = false;

    if (platform.type === "claude-commands") {
      skillCount = skills.filter((s) => s.installedClaude).length;
      // Check for guardrails in CLAUDE.md
      const claudeMd = path.join(targetDir, "CLAUDE.md");
      if (fs.existsSync(claudeMd)) {
        const content = fs.readFileSync(claudeMd, "utf8");
        guardrails = content.includes("# Security Skills for Claude");
      }
    } else {
      const key = "installed_" + platform.key;
      skillCount = skills.filter((s) => s[key]).length;
      // Check guardrails file or append target
      if (platform.guardrailsTarget) {
        guardrails = fs.existsSync(path.join(targetDir, platform.guardrailsTarget));
      } else if (platform.guardrailsAppendTarget) {
        const target = path.join(targetDir, platform.guardrailsAppendTarget);
        if (fs.existsSync(target)) {
          const content = fs.readFileSync(target, "utf8");
          guardrails = content.includes(`# Security Skills for ${platform.name}`);
        }
      }
    }

    platforms[platform.key] = { detected: isDetected, skillCount, guardrails };
  }

  let config = null;
  try {
    config = readConfig(targetDir);
  } catch {
    // config parse error — leave as null
  }

  const totalInstalled = Object.values(platforms).reduce((sum, p) => sum + p.skillCount, 0);

  return { version, platforms, config, totalInstalled };
}

function skillSourcePath(skill) {
  return path.join(PKG_ROOT, "skills", "claude", skill.category, skill.file);
}

function commandSourcePath(skill) {
  return path.join(PKG_ROOT, ".claude", "commands", skill.command);
}

const GUARDRAILS_VERSION = 1;

function guardrailsContent(platformName) {
  const template = fs.readFileSync(path.join(PKG_ROOT, "docs", "guardrails.md"), "utf8");
  return `<!-- skillvault-guardrails:${GUARDRAILS_VERSION} -->\n# Security Skills for ${platformName}\n\n${template}`;
}

/**
 * Returns the version number of the guardrails block embedded in content,
 * 0 if the legacy (unversioned) marker is present, or null if absent.
 */
function getGuardrailsVersion(content) {
  const match = content.match(/<!-- skillvault-guardrails:(\d+) -->/);
  if (match) return parseInt(match[1], 10);
  if (content.includes("# Security Skills for ")) return 0;
  return null;
}

/**
 * Replaces the existing guardrails block (versioned or legacy) with newBlock.
 * Returns the updated string.
 */
function replaceGuardrailsBlock(content, newBlock) {
  const versionedIdx = content.indexOf("<!-- skillvault-guardrails:");
  const legacyIdx = content.indexOf("# Security Skills for ");
  const blockStart = versionedIdx !== -1 ? versionedIdx : legacyIdx;
  if (blockStart === -1) return content + "\n\n" + newBlock;

  // Trim trailing whitespace/newlines before the block to keep clean spacing
  let insertAt = blockStart;
  while (insertAt > 0 && (content[insertAt - 1] === "\n" || content[insertAt - 1] === "\r")) {
    insertAt--;
  }
  return content.slice(0, insertAt) + "\n\n" + newBlock;
}

// ── Integrity Verification ────────────────────────────────────────────────────

const CONFIG_VERSION = 1;

/**
 * Verifies SHA-256 checksums of all bundled skill files against checksums.json.
 * Throws if any file is missing or has been tampered with.
 * No-ops when checksums.json is absent (development/test environments).
 */
function verifyChecksums() {
  const checksumsPath = path.join(PKG_ROOT, "checksums.json");
  if (!fs.existsSync(checksumsPath)) return;

  const checksums = JSON.parse(fs.readFileSync(checksumsPath, "utf8"));

  for (const [relPath, expectedHash] of Object.entries(checksums)) {
    const fullPath = path.join(PKG_ROOT, relPath);
    if (!fs.existsSync(fullPath)) {
      throw new Error(`Integrity check failed: missing bundled file '${relPath}'. The package may be corrupt.`);
    }
    const actualHash = crypto.createHash("sha256").update(fs.readFileSync(fullPath)).digest("hex");
    if (actualHash !== expectedHash) {
      throw new Error(`Integrity check failed: '${relPath}' has been modified. The package may have been tampered with.`);
    }
  }
}

function detectPlatforms(targetDir) {
  const result = {};
  for (const platform of PLATFORMS) {
    const detected = platform.detect.some((d) => {
      const fullPath = path.join(targetDir, d.path);
      return fs.existsSync(fullPath);
    });
    result[platform.key] = detected;
  }
  return result;
}

function filterByCategory(skills, categories) {
  if (!categories || categories.length === 0) return skills;
  const cats = new Set(categories.map((c) => c.toLowerCase()));
  return skills.filter((s) => cats.has(s.category.toLowerCase()));
}

function listSkills(targetDir) {
  const claudeDir = path.join(targetDir, ".claude", "commands");

  return SKILLS.map((skill) => {
    const result = {
      slug: skill.slug,
      name: skill.name,
      category: skill.category,
      installedClaude: fs.existsSync(path.join(claudeDir, skill.command)),
    };

    for (const platform of PLATFORMS) {
      if (platform.type === "claude-commands") continue;
      const dir = path.join(targetDir, platform.skillsDir);
      result["installed_" + platform.key] = fs.existsSync(path.join(dir, skill.file));
    }

    // Keep backward-compatible alias
    result.installedCursor = result.installed_cursor;

    return result;
  });
}

function removeSkills(slugs, targetDir, dryRun) {
  if (dryRun === undefined) dryRun = false;
  const removed = [];
  const modifiedDirs = new Set(); // C4: track dirs where files were removed

  for (const slug of slugs) {
    const skill = SKILLS.find((s) => s.slug === slug);
    if (!skill) continue;

    const claudePath = path.join(targetDir, ".claude", "commands", skill.command);
    if (fs.existsSync(claudePath)) {
      if (!dryRun) fs.unlinkSync(claudePath);
      removed.push(`.claude/commands/${skill.command}`);
      modifiedDirs.add(path.join(targetDir, ".claude", "commands"));
    }

    for (const platform of PLATFORMS) {
      if (platform.type === "claude-commands") continue;
      const filePath = path.join(targetDir, platform.skillsDir, skill.file);
      if (fs.existsSync(filePath)) {
        if (!dryRun) fs.unlinkSync(filePath);
        removed.push(`${platform.skillsDir}/${skill.file}`);
        modifiedDirs.add(path.join(targetDir, platform.skillsDir));
      }
    }
  }

  // C4: remove directories that are now empty
  if (!dryRun) {
    for (const dir of modifiedDirs) {
      if (fs.existsSync(dir) && fs.readdirSync(dir).length === 0) {
        try {
          fs.rmdirSync(dir);
          const rel = path.relative(targetDir, dir).replace(/\\/g, "/");
          removed.push(`${rel}/ (empty directory removed)`);
        } catch {
          // ignore — directory may have been removed by a prior iteration
        }
      }
    }
  }

  return removed;
}

function readConfig(targetDir) {
  const configPath = path.join(targetDir, ".skillvaultrc");
  if (!fs.existsSync(configPath)) return null;

  let data;
  try {
    data = JSON.parse(fs.readFileSync(configPath, "utf8"));
  } catch {
    throw new Error(".skillvaultrc contains invalid JSON");
  }

  if (data.version === undefined) {
    process.stderr.write("  Warning: .skillvaultrc has no 'version' field. Run 'skillvault init --save' to update it.\n");
  } else if (data.version !== CONFIG_VERSION) {
    process.stderr.write(`  Warning: .skillvaultrc version ${data.version} is not supported (expected ${CONFIG_VERSION}). Some features may not work correctly.\n`);
  }

  if (data.skills) {
    if (!Array.isArray(data.skills)) {
      throw new Error(".skillvaultrc: 'skills' must be an array");
    }
    const validSlugs = new Set(SKILLS.map((s) => s.slug));
    for (const slug of data.skills) {
      if (!validSlugs.has(slug)) {
        throw new Error(`.skillvaultrc: unknown skill slug '${slug}'`);
      }
    }
  }

  if (data.platform) {
    const validKeys = new Set(PLATFORMS.map((p) => p.key));
    validKeys.add("both");
    if (Array.isArray(data.platform)) {
      for (const p of data.platform) {
        if (!validKeys.has(p)) {
          throw new Error(`.skillvaultrc: unknown platform '${p}'`);
        }
      }
    } else if (!validKeys.has(data.platform)) {
      throw new Error(`.skillvaultrc: platform must be one of: ${[...validKeys].join(", ")}`);
    }
  }

  return data;
}

function writeConfig(config, targetDir) {
  const configPath = path.join(targetDir, ".skillvaultrc");
  const withVersion = { version: CONFIG_VERSION, ...config };
  fs.writeFileSync(configPath, JSON.stringify(withVersion, null, 2) + "\n");
  fs.chmodSync(configPath, 0o644);
  return configPath;
}

function installClaude(selectedSkills, includeGuardrails, targetDir, dryRun, force) {
  if (dryRun === undefined) dryRun = false;
  if (force === undefined) force = false;
  verifyChecksums();
  const commandsDir = path.join(targetDir, ".claude", "commands");
  // S3: explicit directory permissions
  if (!dryRun) fs.mkdirSync(commandsDir, { recursive: true, mode: 0o755 });

  const copied = [];

  for (const skill of selectedSkills) {
    const src = commandSourcePath(skill);
    const dest = path.join(commandsDir, skill.command);
    const relPath = `.claude/commands/${skill.command}`;

    // C6: warn and skip if the installed file has been locally modified
    if (!force && !dryRun && fs.existsSync(dest)) {
      const srcHash = crypto.createHash("sha256").update(fs.readFileSync(src)).digest("hex");
      const destHash = crypto.createHash("sha256").update(fs.readFileSync(dest)).digest("hex");
      if (srcHash !== destHash) {
        process.stderr.write(`  Warning: ${relPath} has local modifications — skipping. Use --force to overwrite.\n`);
        copied.push(`${relPath} (modified — skipped)`);
        continue;
      }
    }

    if (!dryRun) { fs.copyFileSync(src, dest); fs.chmodSync(dest, 0o644); }
    copied.push(relPath);
  }

  if (includeGuardrails) {
    const src = path.join(PKG_ROOT, "Claude.md");
    const dest = path.join(targetDir, "CLAUDE.md");
    // Prepend the version tag so we can detect and replace stale guardrails on update
    const guardrails = `<!-- skillvault-guardrails:${GUARDRAILS_VERSION} -->\n` + fs.readFileSync(src, "utf8");

    if (fs.existsSync(dest)) {
      const existing = fs.readFileSync(dest, "utf8");
      const existingVersion = getGuardrailsVersion(existing);

      if (existingVersion === GUARDRAILS_VERSION) {
        copied.push("CLAUDE.md (guardrails already up to date)");
      } else if (existingVersion !== null) {
        // Old or unversioned guardrails — replace the block in place
        if (!dryRun) { fs.writeFileSync(dest, replaceGuardrailsBlock(existing, guardrails)); fs.chmodSync(dest, 0o644); }
        copied.push(`CLAUDE.md (guardrails updated to v${GUARDRAILS_VERSION})`);
      } else {
        if (!dryRun) fs.appendFileSync(dest, "\n\n" + guardrails);
        copied.push("CLAUDE.md (appended guardrails)");
      }
    } else {
      if (!dryRun) { fs.writeFileSync(dest, guardrails); fs.chmodSync(dest, 0o644); }
      copied.push("CLAUDE.md");
    }
  }

  return copied;
}

function installRulesDir(platform, selectedSkills, includeGuardrails, targetDir, dryRun, force) {
  if (dryRun === undefined) dryRun = false;
  if (force === undefined) force = false;
  verifyChecksums();
  const rulesDir = path.join(targetDir, platform.skillsDir);
  // S3: explicit directory permissions
  if (!dryRun) fs.mkdirSync(rulesDir, { recursive: true, mode: 0o755 });

  const copied = [];

  for (const skill of selectedSkills) {
    const src = skillSourcePath(skill);
    const dest = path.join(rulesDir, skill.file);
    const relPath = `${platform.skillsDir}/${skill.file}`;

    // C6: warn and skip if the installed file has been locally modified
    if (!force && !dryRun && fs.existsSync(dest)) {
      const srcHash = crypto.createHash("sha256").update(fs.readFileSync(src)).digest("hex");
      const destHash = crypto.createHash("sha256").update(fs.readFileSync(dest)).digest("hex");
      if (srcHash !== destHash) {
        process.stderr.write(`  Warning: ${relPath} has local modifications — skipping. Use --force to overwrite.\n`);
        copied.push(`${relPath} (modified — skipped)`);
        continue;
      }
    }

    if (!dryRun) { fs.copyFileSync(src, dest); fs.chmodSync(dest, 0o644); }
    copied.push(relPath);
  }

  if (includeGuardrails) {
    const content = guardrailsContent(platform.name);
    const marker = `# Security Skills for ${platform.name}`;

    // Append-to-file pattern (Copilot, Codex, Augment)
    if (platform.guardrailsAppendTarget) {
      const dest = path.join(targetDir, platform.guardrailsAppendTarget);
      if (fs.existsSync(dest)) {
        const existing = fs.readFileSync(dest, "utf8");
        const existingVersion = getGuardrailsVersion(existing);

        if (existingVersion === GUARDRAILS_VERSION) {
          copied.push(`${platform.guardrailsAppendTarget} (guardrails already up to date)`);
        } else if (existingVersion !== null) {
          if (!dryRun) { fs.writeFileSync(dest, replaceGuardrailsBlock(existing, content)); fs.chmodSync(dest, 0o644); }
          copied.push(`${platform.guardrailsAppendTarget} (guardrails updated to v${GUARDRAILS_VERSION})`);
        } else {
          if (!dryRun) fs.appendFileSync(dest, "\n\n" + content);
          copied.push(`${platform.guardrailsAppendTarget} (appended guardrails)`);
        }
      } else {
        if (!dryRun) { fs.writeFileSync(dest, content); fs.chmodSync(dest, 0o644); }
        copied.push(platform.guardrailsAppendTarget);
      }
    }

    // Copy-to-target pattern (all other rules-dir platforms)
    if (platform.guardrailsTarget) {
      const dest = path.join(targetDir, platform.guardrailsTarget);
      if (!dryRun) { fs.writeFileSync(dest, content); fs.chmodSync(dest, 0o644); }
      copied.push(platform.guardrailsTarget);
    }
  }

  return copied;
}

// Backward-compatible wrapper
function installCursor(selectedSkills, includeGuardrails, targetDir, dryRun, force) {
  const platform = PLATFORMS.find((p) => p.key === "cursor");
  return installRulesDir(platform, selectedSkills, includeGuardrails, targetDir, dryRun, force);
}

function installPlatform(platformKey, selectedSkills, includeGuardrails, targetDir, dryRun, force) {
  const platform = PLATFORMS.find((p) => p.key === platformKey);
  if (!platform) throw new Error(`Unknown platform: ${platformKey}`);

  if (platform.type === "claude-commands") {
    return installClaude(selectedSkills, includeGuardrails, targetDir, dryRun, force);
  }
  return installRulesDir(platform, selectedSkills, includeGuardrails, targetDir, dryRun, force);
}

module.exports = {
  SKILLS,
  PLATFORMS,
  PKG_ROOT,
  CONFIG_VERSION,
  GUARDRAILS_VERSION,
  getVersion,
  verifyChecksums,
  getGuardrailsVersion,
  replaceGuardrailsBlock,
  doctor,
  skillSourcePath,
  commandSourcePath,
  guardrailsContent,
  detectPlatforms,
  filterByCategory,
  listSkills,
  removeSkills,
  readConfig,
  writeConfig,
  installClaude,
  installCursor,
  installRulesDir,
  installPlatform,
};
